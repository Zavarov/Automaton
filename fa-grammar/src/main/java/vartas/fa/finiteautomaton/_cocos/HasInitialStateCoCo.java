 ...
 */
public class HasInitialStateCoCo implements FiniteAutomatonASTFiniteAutomatonCoCo {
    /**
     * The error message that is throw.
     */
    public static final String ERROR_MESSAGE = "This automaton doesn't have an initial state.";
    ...
    public void check(ASTFiniteAutomaton node){
        Optional<ASTState> state = node.getStateList().stream().filter(ASTState::isInitial).findAny();
        if(!state.isPresent())
